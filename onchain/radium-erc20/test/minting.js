...
describe(constants.TOKEN_CONTRACT_ID + ": Minting", function () {
    ...
    describe("Total Minted", function () {
        it("totalMinted increments", async function () {
            ...
            expect(await token.totalSupply()).to.equal(await token.totalMinted()); 
            expect(await token.totalMinted()).to.equal(constants.INITIAL_SUPPLY + (quantity*2));
        });

        it("totalMinted does not clear on burnAll", async function () {
            const quantity = 100;
            await token.mint(owner.address, quantity);
            await token.mint(addr1.address, quantity);
            ...
            await token.mint(addr1.address, quantity);
            expect(await token.totalMinted()).to.equal(constants.INITIAL_SUPPLY + (quantity * 3));
        });
    });
